...
> drafting, underwriting, briefs, `learn()`) are running blind or on fallbacks. Heartbeats stay
> green because they wrap the loop, not the Claude call. Top-up = owner action (P0-1).
...
| `solomon` | `daycare_director.py` | 900 s tick, brief every 24 h | yes | none | `solomon` | **RED** — errStreak ~4.9 k (credits) + retry storm (no backoff) + invalid Meta token; last brief 2026-08-01 |
...
| **ACE** | Wholesale | per-thread state machine; reply-vs-escalate; call-ready queue | **modes off/shadow/supervised/full — supervised/full auto-send.** Default off. *Not in CLAUDE.md — document it.* |
...
| **Orion** | Cross-business | daily "attack today" CEO brief on Mission Control | Telegram push optional. *Not in CLAUDE.md or hub roster.* |
...
Smallest fix: extend `forge_heartbeat.beat()` with `lastSuccessAt` / `errorsTotal` / `lastError`
(every loop already calls it) and have `review_agent._claude` record a global AI-health signal.
...
| DEGRADED | errStreak ≥ 1 or AI dependency failing (**new**) |
