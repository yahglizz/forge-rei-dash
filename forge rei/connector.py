...
class Handler(BaseHTTPRequestHandler):
    ...
    def _handle_dropship_post(self, path):
        ...
        try:
            ...
            elif path == "/api/dropship/adspy/search":
                keyword = str(body.get("q") or "").strip()
                if not keyword:
                    result = {"ok": False, "error": "q (keyword) required",
                              "code": "bad_request", "ads": [], "winners": []}
                else:
                    try:
                        ...
                        limit = None
                    try:
                        min_days = max(0, int(body.get("min_days", 21)))
                    except Exception:
                        min_days = 21
                    result = dropship_adspy.search(
                        keyword, country=str(body.get("country") or "").strip() or None,
                        limit=limit)
                    result["minDays"] = min_days
                    result["winners"] = dropship_adspy.winners(
                        result.get("ads") or [], min_days=min_days)
            elif path == "/api/dropship/adspy/advertiser":
                page = str(body.get("page") or "").strip()
                if not page:
                    result = {"ok": False, "error": "page required", "code": "bad_request",
                              "ads": []}
                else:
                    try:
                        limit = int(body["limit"]) if "limit" in body else None
