...
try:
    ...
    check(r1 is False and r3 is False, "unwritable path -> False, no exception")
    check(r2 is False, "non-dict result on unwritable path -> False")
    check(time.time() - t0 < 1.0, "failures return fast")
...
# 6. a held lock never stalls the caller (a send path must not wait on the log)
fresh("locked")
action_log._LOCK.acquire()
try:
    t0 = time.time()
    r = action_log.record("marcus", "approve_send")
    check(r is False and time.time() - t0 < 1.0, "lock contention -> dropped, not blocked")
finally:
    action_log._LOCK.release()
