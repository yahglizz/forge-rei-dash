...
### [D] Copy Writer — 2026-08-21 — ACCEPTED, with one framing correction

Verified by the head agent: 189 warm rows → **91 send · 10 nurture · 88 exclude**.
**0 price/offer/range violations across all 91 sendable drafts** (independent regex scan for
`$`, 3+ digit runs, "offer", "ballpark", "range"). 0 sendable drafts with empty grounding.
6 bulk templates, all GSM-7, all exactly 2 segments.

**Sharp catch on the footer:** our own opt-out language uses a curly apostrophe `’`, which is
not in GSM-7 and silently forces UCS-2 — turning a 2-segment body into 5. On `never_replied`
alone that is 5,646 → 14,115 billed segments. D swapped it for a straight `'`.

**88 of the 189 warmest leads should NOT be texted** — wrong numbers, flat refusals,
hostility, a carrier bounce, and **4 who are BUYERS, not sellers** (`ttb7dvGh1bI76CDvnzVc`
"I do not sell I only buy", + `XkB3meE7vHESoAr0VGXH`, `IJxKPwtFrs4NaJTFrqxa`,
`AylYmth33nB6XJ81I0Ie`). **Stage E's send files must filter on the `recommendation` column,
not row count** — E built them from the 189, so the GHL_IMPORT warm files are currently
88 rows too permissive. Head agent to reconcile before any send.

**Correction to D's framing of the credit finding.** D wrote "Marcus is drafting every seller
reply from static templates right now." Precisely: the SMS drafting loop is **not running at
all** (`FORGE_MARCUS_SMS` unset → `MARCUS.run_forever` never starts), so nothing is being
silently templated today. The silent fallback is **latent** — it fires the moment the
operator presses "Hand to Marcus" or enables the flag. That makes it a pre-send blocker
rather than an active incident, but it does NOT reduce its severity: enabling
`FORGE_MARCUS_SMS` while the key is dead produces static templates for 3,022 leads with the
failure visible only in `engine.last_error`.

**Provenance caveat, stated plainly:** because both API keys are dead, these 189 drafts were
written by the Stage D agent against each seller's real thread text, NOT generated by
`marcus_engine._ai_draft`'s own Claude call. Every safety gate is production
(`_scrub_voice` → `_no_price_over_text` → `_draft_safety_reason` →
`sms_guard._quotes_price_or_offer`), but the *generation* is not the production voice engine.
Re-run `reengage_copy.py` once the key is funded to regenerate through the real path.

## 9. ROOT CAUSE — why wholesale went dry (found 2026-08-21)

Not one thing. Four, stacked:

1. **Both Anthropic API keys have been out of credit since ~2026-08-01.** Verified live:
   `marcus-wholesale-agent/config/ghl.env` and `forge-agency/config/agency.env` both return
   `HTTP 400: "Your credit balance is too low to access the Anthropic API."` Box
   `cost_tracker.json` last recorded day is **2026-08-01** — 20 days of no Claude spend.
   Every AI agent in the system has been dead that whole time: Scout scoring, Marcus
   drafting, Atlas underwriting, Solomon, Midas, and every self-improvement loop.
2. **The 194 Ohio re-engage drafts written 2026-08-02 were never sent** (§2).
3. **`FORGE_MARCUS_SMS` is unset**, so inbound replies never auto-produce a draft.
4. **A large unactioned proposal backlog** is sitting in `proposals.jsonl`.

The date alignment is not a coincidence: credit died ~Aug 1, the last re-engage work stopped
Aug 2, and nothing has moved since.
